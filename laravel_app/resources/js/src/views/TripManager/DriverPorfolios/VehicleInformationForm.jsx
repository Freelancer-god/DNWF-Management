...
function VehicleInformationForm({ record, form, submitedForms }) {
  ...
  useEffect(() => {
    if (vehicleModelList.length === 1) {
      const fetchData = async () => {
        const dataToSend = {
          filter: {
            vehicle_brand_id: form.getFieldValue('vehicle_brand_id'),
          },
        };
  ...
  useEffect(() => {
    if (!vehiclePhoto || !vehiclePhoto.medias || vehiclePhoto.medias.length === 0) return;
    axios
  ...
  const renderConfirmRadio = () => (
    <>
      ...
      >
        <Radio.Group value={null} onChange={(e) => setStatusType1(e.target.value)} disabled={vehicleInfo.status !== DRIVER_FORM_STATUS.NOT_VERIFIED}>
      ...
      {form.getFieldValue('status_type_1') === DRIVER_FORM_STATUS.DENINED && (
        ...
        >
          <TextArea rows={2} maxLength={200} placeholder={tt('Nhập lý do vì sao không hợp lệ')} allowClear disabled={vehicleInfo.status !== DRIVER_FORM_STATUS.NOT_VERIFIED} />
        </Form.Item>
      )}
      ...
      >
        <Radio.Group value={null} onChange={(e) => setStatusType2(e.target.value)} disabled={vehiclePhoto.status !== DRIVER_FORM_STATUS.NOT_VERIFIED}>
          <Radio value={DRIVER_FORM_STATUS.VERIFIED}>{tt('Hợp lệ')}</Radio>
          <Radio value={DRIVER_FORM_STATUS.DENINED}>{tt('Không hợp lệ')}</Radio>
        </Radio.Group>

      </Form.Item>
      {form.getFieldValue('status_type_2') === DRIVER_FORM_STATUS.DENINED && (
        <Form.Item
          label={tt('Lý do')}
          name="note_type_2"
          rules={[
        ...
        >
          <TextArea rows={2} maxLength={200} placeholder={tt('Nhập lý do vì sao không hợp lệ')} allowClear disabled={vehiclePhoto.status !== DRIVER_FORM_STATUS.NOT_VERIFIED} />
        </Form.Item>
      )}
      ...
      >
        <Radio.Group value={null} onChange={(e) => setStatusType3(e.target.value)} disabled={vehicleInsure.status !== DRIVER_FORM_STATUS.NOT_VERIFIED}>
          <Radio value={DRIVER_FORM_STATUS.VERIFIED}>{tt('Hợp lệ')}</Radio>
          <Radio value={DRIVER_FORM_STATUS.DENINED}>{tt('Không hợp lệ')}</Radio>
        </Radio.Group>
      ...
      {form.getFieldValue('status_type_3') === DRIVER_FORM_STATUS.DENINED && (
        ...
        >
          <TextArea rows={2} maxLength={200} placeholder={tt('Nhập lý do vì sao không hợp lệ')} allowClear disabled={vehicleInsure.status !== DRIVER_FORM_STATUS.NOT_VERIFIED} />
        </Form.Item>
      )}
  ...
  const handleOnVehicleModelChange = async (vehicle_brand_model_id, record) => {
    form.setFieldsValue({
  ...
  const renderDriverInfo = () => (
    (
      <>
        ...
        >
          <Select placeholder={tt('Chưa cập nhật')} onChange={handleOnVehicleTypeChange}>
        ...
        >
          <Select placeholder={tt('Chưa cập nhật')} onChange={handleOnVehicleModelChange}>
  ...
  const renderDriverBHBB = () => (
    <>
      <Form.Item
        label={tt('Ngày hết hạn BHBB TNDS  ')}
        name="insurance_civil_accident_date"
        rules={[
          {
            required: form.getFieldValue('status_type_3') !== DRIVER_FORM_STATUS.DENINED,
            message: tt('Vui lòng chọn ngày hết hạn '),
          },
          ({ getFieldValue }) => ({
            validator(rule, value) {
              if (value < dayjs()) {
                return Promise.reject(tt('Ngày hết hạn phải sau ngày hiện tại '));
              }
              return Promise.resolve();
            },
          }),
      ...
      <Form.Item
        label={tt('Giá trị mua BHBB TNDS')}
        name="insurance_civil_accident_price"
        rules={[
          {
            required: form.getFieldValue('status_type_3') !== DRIVER_FORM_STATUS.DENINED,
            message: tt('Vui lòng điền số tiền mua BHBB TNDS'),
          },
          ({ getFieldValue }) => ({
            validator(rule, value) {
              if (value < 50000) {
                return Promise.reject(tt('Số tiền mua BHBB TNDS phải lơn hơn 50.000 VNĐ'));
              }
              return Promise.resolve();
            },
          }),
      ...
      <div className="flex flex-col">
        ...
        <div className="mt-[20px] flex flex-wrap md:flex-nowrap items-start gap-[10px]">
          ...
          >
            ...
            {vehiclePhoto && vehicleInfo && vehicleInsure && renderConfirmRadio()}

          </Form>
          <div className="overflow-hidden relative">
            {vehicleInfo && (<PhotosViewer photos={vehicleInfo?.medias} />)}
            {vehiclePhoto && (<PhotosViewer photos={vehiclePhoto?.medias} />)}
            {vehicleInsure && (<PhotosViewer photos={vehicleInsure?.medias} />)}
          </div>
        </div>
      </div>
