...
import { DRIVER_FORM_STATUS, PERMISSIONS_MAP, PROFILE_MAP } from '../../../constants';
import DriverProfileForm from './DriverProfileForm';
import { updateDriverForms } from '../../../store/DriverManager/API';
...
function DriverProfile({ data, submitedForms, onUpdate }) {
  ...
  const generateDataToSend = (values, saveOnly = false) => {
    const {
      ...
      citizen_name, place_of_origin, place_of_residence, birth_date,
      sex, status_type_1, status_type_3, note_type_1, note_type_3,
    } = values;
    const generatedData = {
      driver_id: data.id,
      form_list: [{
        id: submitedForms[PROFILE_MAP.CCCD] ? submitedForms[PROFILE_MAP.CCCD].id : null,
        data: {
          citizen_identify,
          citizen_expire_date: citizen_expire_date ? citizen_expire_date.format('YYYY-MM-DD') : null,
          citizen_issued_date: citizen_issued_date ? citizen_issued_date.format('YYYY-MM-DD') : null,
          citizen_name,
          place_of_origin,
          place_of_residence,
      ...
      {
        id: submitedForms[PROFILE_MAP.ANH] ? submitedForms[PROFILE_MAP.ANH].id : null,
        notes: note_type_3,
      },
      ],
    };
    ...
    if (status_type_1 !== undefined || status_type_3 !== undefined) {
      if (saveOnly) {
        generatedData.form_list[0].status = submitedForms[PROFILE_MAP.CCCD].status;
        generatedData.form_list[1].status = submitedForms[PROFILE_MAP.ANH].status;
      ...
      return [
        ...
        <div>
          <div className="text-gray-500 text-[11px] mr-[10px] min-w-[100px]">
            {tt('Ngày gửi hồ sơ: ')}
            {submitedForms[PROFILE_MAP.CCCD].created_at ? formatNgayThang(submitedForms[PROFILE_MAP.CCCD].created_at, true) : tt('(Chưa cập nhật)')}
          </div>
          <div className="text-gray-500 text-[11px] mr-[10px] min-w-[100px]">
            {tt('Ngày cập nhật hồ sơ: ')}
            {submitedForms[PROFILE_MAP.CCCD].updated_at ? formatNgayThang(submitedForms[PROFILE_MAP.CCCD].updated_at, true) : tt('(Chưa cập nhật)')}
          </div>
        </div>,
      ];
    }
  ...
  return (
    <div>
      <div className="flex flex-wrap gap-[10px]">
        <div className="max-w-[350px] overflow-hidden break-words mr-[5px]">
          ...
          <div className="flex flex-row mb-0 ">
            ...
            <div>{data?.birth_date ? formatNgayThang(data?.birth_date) : tt('(Chưa cập nhật)')}</div>
          </div>
          <div className="flex flex-row mb-0 ">
            <div className="text-gray-500 text-[11px] mr-[10px] min-w-[120px]">
              {tt('Giới tính:')}
            </div>
            <div>{data?.sex !== null ? (data?.sex === 0 ? tt('Nữ') : tt('Nam')) : tt('(Chưa cập nhật)')}</div>
          </div>
        ...
        <div className="max-w-[500px] overflow-hidden break-words mr-[5px] relative">
          {decodedMedia && (<PhotosViewer photos={decodedMedia[PROFILE_MAP.CCCD]} />)}
        </div>
      ...
      >
        ...
        {isModalOpen && <DriverProfileForm record={decodedMedia} form={form} submitedForms={submitedForms} />}
