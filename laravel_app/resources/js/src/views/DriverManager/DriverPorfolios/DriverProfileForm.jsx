  ...
  useEffect(() => {
    ...
    const fieldsValue = {
      ...
      citizen_expire_date: citizen_expire_date
        ? new dayjs(citizen_expire_date)
        : null,
      citizen_issued_date: citizen_issued_date
        ? new dayjs(citizen_issued_date)
        : null,
      citizen_name,
      place_of_origin,
      place_of_residence,
      ...
      ...(driverProfile.status !== DRIVER_FORM_STATUS.NOT_VERIFIED
        ? { status_type_1: driverProfile.status }
        : { status_type_1: null }),
      ...(driverPhoto.status !== DRIVER_FORM_STATUS.NOT_VERIFIED
        ? { status_type_3: driverPhoto.status }
        : { status_type_3: null }),
    };
          ...
          ({ getFieldValue }) => ({
            validator(rule, value) {
              if (
                ...
                // if (value?.length > 0 && value?.trim() === '') return Promise.reject(tt('Vui lòng điền thông tin độ dài ít nhất là 9 ký tự'));
                if (
                  value?.length > 0
                                    && value?.trim().length < 9
                ) {
      ...
      <Form.Item
        ...
        rules={[
          ...
          ({ getFieldValue }) => ({
            validator(rule, value) {
              if (
                getFieldValue('citizen_expire_date') !== null
                                && value > getFieldValue('citizen_expire_date')
              ) {
                return Promise.reject(
                  tt('Ngày ngày cấp phải trước ngày hết hạn '),
                );
              }
              return Promise.resolve();
            },
          ...
          ({ getFieldValue }) => ({
            validator(rule, value) {
              if (
                ...
                if (
                  value < getFieldValue('citizen_issued_date')
                ) {
                  return Promise.reject(
                    tt('Ngày hết hạn phải sau ngày cấp '),
        ...
        <div className="flex flex-wrap items-start w-[150px] justify-center">
          <img
            ...
            onError={({ currentTarget }) => {
              ...
              currentTarget.src = record?.sex === 0
                ? avatarFemaleDefault
                : avatarMaleDefault;
            }}
