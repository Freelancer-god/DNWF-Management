...
import {
  DRIVER_FORM_STATUS, PERMISSIONS_MAP, PROFILE_MAP, VEHICLE_INFOMATION_TYPE, VEHICLE_MAP,
} from '../../../constants';
...
function VehicleRegistration({
  ...
  const generateDataToSend = (values, saveOnly = false) => {
    const {
      status_type_4, note_type_4,
    } = values;
    const generatedData = {
      driver_id: userId,
      form_list: [{
        id: submitedForms[VEHICLE_INFOMATION_TYPE.GCNDK] ? submitedForms[VEHICLE_INFOMATION_TYPE.GCNDK].id : null,
    ...
    if (status_type_4 !== undefined) {
      if (saveOnly) {
        generatedData.form_list[0].status = submitedForms[VEHICLE_INFOMATION_TYPE.GCNDK].status;
      ...
      return [
        ...
        <div>
          <div className="text-gray-500 text-[11px] mr-[10px] min-w-[100px]">
            {tt('Ngày gửi hồ sơ: ')}
            {submitedForms[VEHICLE_INFOMATION_TYPE.DKX].created_at ? formatNgayThang(submitedForms[VEHICLE_INFOMATION_TYPE.DKX].created_at, true) : tt('(Chưa cập nhật)')}
          </div>
          <div className="text-gray-500 text-[11px] mr-[10px] min-w-[100px]">
            {tt('Ngày cập nhật hồ sơ: ')}
            {submitedForms[VEHICLE_INFOMATION_TYPE.DKX].updated_at ? formatNgayThang(submitedForms[VEHICLE_INFOMATION_TYPE.DKX].updated_at, true) : tt('(Chưa cập nhật)')}
          </div>
        </div>,
      ];
    }
  ...
  return (
    <>
      ...
      <Modal
        title={tt('Duyệt / Cập nhật thông tin giấy chứng nhận đăng kiểm')}
        open={isModalOpen}
        onOk={handleOk}
        style={{ top: 20 }}
        width="100vw"
      ...
      >
        ...
        {isModalOpen && <VehicleRegistrationForm record={decodedMedia} form={form} submitedForms={submitedForms} />}
      </Modal>
    </>
  );
}
