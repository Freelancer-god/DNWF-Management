...
import { exportVehicles } from "../../store/VehicleManager/API";
import {
    PERMISSIONS_MAP,
    VEHICLE_MAP,
    VEHICLE_MAP_W_COLOR,
...
function CustomList() {
    ...
    const initColumns = [
        ...
        {
            title: tt("Số điện thoại"),
            label: tt("Số điện thoại"),
            key: "driver_phone",
        ...
        {
            title: tt("Số chỗ ngồi"),
            label: tt("Số chỗ ngồi"),
            key: "seats",
            dataIndex: "seats",
            width: 120,
        ...
        //           <Tag icon={icon} color={TRIP_STATUS_MAP_W_COLOR[value].color}>
        ...
        //       <AntdButton type="link" onClick={() => { setCurrentTrip(item); setIsModalOpen(true); }}>
        //         {tt('Xem bản đồ')}
        //       </AntdButton>
        //       {myPermissions[PERMISSIONS_MAP['cancel-trip']] && item.status !== TRIP_STATUS_MAP_TEXT['Bị huỷ'] && item.status !== TRIP_STATUS_MAP_TEXT['Đã kết thúc'] && (
        //       <AntdButton type="link" onClick={() => { setCurrentTrip(item); setIsConfirmModalOpen(true); }}>
        //         {tt('Huỷ chuyến')}
        //       </AntdButton>
        //       )}
    ...
    const listField = [
        { label: tt("Mã ref"), value: "vehicles.reference", type: "input" },
        {
            label: tt("Mã tài xế"),
            value: "vehicles.driver_reference",
            type: "input",
        },
        {
            label: tt("SĐT tài xế"),
            value: "vehicles.driver_phone",
            type: "input",
        },
        {
            label: tt("Tên chủ xe"),
            value: "vehicles.owner_name",
            type: "input",
        },
        {
            label: tt("Địa chỉ"),
            value: "vehicles.vehicle_address",
            type: "input",
        },
        {
            label: tt("Biển kiểm soát"),
    ...
    const breadcrumbItems = [
        {
            title: (
                <Link to="/cms/vehicles">{tt("Danh sách phương tiện")}</Link>
            ),
    ...
    const handleOnPressDownload = async () => {
        ...
        const path = await exportVehicles(dataToSend);
        if (path) {
