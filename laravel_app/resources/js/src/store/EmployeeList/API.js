...
import { clog, encodedData, tt } from '../../utils';
...
export const fetchGetList = (
    ...
    const str = document.getElementById("myUser")?.value;
    const userJson = encodedData(str)
    const dataToSend = {
        term, page, limit, filter: {
            except_ids: [userJson.id]
        }, order_by: order, sort,
    };
    ...
    return axios
        .post(generateUrl('api/cmsService/v1/employees/search'), dataToSend, config)
        .then((res) => res.data)
        .then((data) => {
...
export const changeMyPass = ({ password, old_password }) => {
    const dataToSend = { password, old_password };
    const config = {
        headers: { Authorization: `Bearer ${document.getElementById('myToken')?.value}` },
    };

    return axios
        .post(generateUrl('api/cmsService/v1/employees/changePassword'), dataToSend, config)
        .then((res) => res.data)
        .then((data) => {
            if (data.success === false) {
                showError(data.error);
